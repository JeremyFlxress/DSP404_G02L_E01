...
namespace SitioWebdeFP240479_New.Models
{
    public class conexion
    {
        ...
        {
            this.conexionSQL.Close(); //Cierra la conexion a la base de datos
        }
        ...
        public Datos MostrarOferta(int idOferta)
        {
            ...
            try
            {
                ...
                if (lector.HasRows)
                {
                    ...
                    lector.Close();
                    desconectar();
                    return items;  // Retorna los datos si se encuentran
                }
                else
                {
                    lector.Close();
                    desconectar();
                    return null; // Si no hay filas, retornar null o un objeto vacío
                }
            }
            catch (SqlException error)
            {
                desconectar();
                // Opción: Puedes loguear el error aquí
 ...
 String fechaContratacion, int nVacantes)
        {
            ...
            String queryUpdate = "update ofertas set nIdEmpresa = @p1,sTituloOferta = @p2,nTipoJornada = @p3,nTipoContrato = @p4,fSalario = @p5,nIdLocalidad = @p6, nTipoLocalidad = @p7,nIdCategoria = @p8,sDescripcion = @p9,fechaContratacion = @p10,nVacantes = @p11 where nIdOferta = " + idOferta;
 SqlCommand comando = new SqlCommand();
            comando.CommandType = System.Data.CommandType.Text;
